...
void Shop::ManageShop(Character& character)
{
	...
	PurchaseLog = "어떤 물품을 원하시나요?   [ <- : 이전 페이지. ] [ -> : 다음 페이지. ] [ z : 구매 ] [ x : 뒤로 가기 ]";
		
	SellLog = "어떤 물품을 파실건가요?   [ <- : 구매한다. ] [ -> : 판매한다. ] [ z : 판매 ] [ x : 뒤로 가기 ]";
...
//		std::cin >> Choice;
		...
		case EKey::LeftArrow: // 구매
			...
			case EKey::RightArrow: // 판매
				...
				{
					UI->AddMessageToBasicCanvasEventInfoUI(L"인벤토리가 비어있습니다.");
					Sleep(500);
				}
				else 
				{
...
void Shop::Display(int index)
{
	...
	ItemNameLog = std::to_string(index + 1) + ". Name: " + ItemsForSale[index]->GetName() + "  Price: " + std::to_string(ItemsForSale[index]->GetPrice()) + "  Effect: " + ItemsForSale[index]->GetExplanation() + "   [ <- : 이전 페이지. ] [ -> : 다음 페이지. ] [ z : 구매 ] [ x : 뒤로 가기 ]  ";
	//	ItemPriceLog = 
